...
`v1.5.5` 的规则很明确：不要用更小、更薄的重写替代已经成熟的 `v1.4.0`
...
当前规划目标是 `1.5.5`。runtime 发布线直接跳到 `1.5.5`；跳过的 patch
号不是独立 runtime 里程碑，也不能变成并行规划线。这个版本升级的是机制，
...
显得更自主、但不提升控制、测量、回放或加固能力的功能，不进入 `v1.5.5` 发布宣称。
...
任何 `v1.5.5` 设计或实现如果偏离这些依据，必须写明原因、风险，以及证明该偏离对 Cortex 更安全的测试。
...
对 `v1.5.5` 来说，这是边界条件，不是营销表述。Cortex 不应说自己实现了生物学认知或
...
定义 `v1.5.5` 的评审意见包含二十五个必做领域。下面的范围矩阵就是它们的权威覆盖面：
...
这张表是 `v1.5.5` 的追踪面。每一行都是必须被规划、实现、文档和验收覆盖的方向，不能静默遗漏。
...
`v1.5.5` 必须用代码级证据逐项验证，不能把之前 `1.5.x` 的描述直接当成已验收事实。
...
P1 工作仍属于 `v1.5.5` 范围，但必须由代码级验收测试支撑，并且不能削弱发布 gate。
...
`v1.5.5` 应按下面顺序推进。这个顺序来自研究依据：认知依赖 grounded observation、
...
`v1.5.5` 不应在 P0 工作完成实现、文档和测试覆盖前发布。范围矩阵中的每一项都必须在发布评审时给出明确状态：已实现、部分实现且列出限制、或作为非发布宣称有意延后。静默遗漏即发布阻断。

工作发布审计表见 [`release-audit-1.5.5.md`](release-audit-1.5.5.md)。这张表是规划和实现之间的交接面；如果某一行仍处于部分完成或发布阻断状态，release notes 不能宣称该项已经完成。
