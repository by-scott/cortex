...
这份文档定义当前 Cortex 发布线。它不是日期承诺，而是 `v1.5.11` 的工程规划契约。

`v1.5.11` 的规则很明确：保持 harness 的强度，关闭既有清理线，并把当前 runtime contract 在 provider-cache-friendly Executive assembly 与 token 可观测性上打磨得更清晰。Cortex 现在被定位为语言模型 harness：用于驱动、观察、回放、评估和加固模型行为的受控表面。本版本应把现有认知机制升级为更强的 harness contract：证据化、类型化、可校准、可回放、可审计、可评估。

## 发布目标

当前规划目标是 `1.5.11`。它是当前 1.5 发布目标，不是并行 roadmap。这个版本升级的是机制，不是概念命名。一个工作项只有在强化下列性质时才进入范围：
...
显得更自主、但不提升控制、测量、回放或加固能力的功能，不进入 `v1.5.11` 发布宣称。
...
任何 `v1.5.11` 设计或实现如果偏离这些依据，必须写明原因、风险，以及证明该偏离对 Cortex 更安全的测试。
...
对 `v1.5.11` 来说，这是边界条件，不是营销表述。Cortex 不应说自己实现了生物学认知或
...
定义 `v1.5.11` 的评审意见包含二十五个必做领域。下面的范围矩阵就是它们的权威覆盖面：
...
这张表是 `v1.5.11` 的追踪面。每一行都是必须被规划、实现、文档和验收覆盖的方向，不能静默遗漏。
...
`v1.5.11` 必须用代码级证据逐项验证，不能把之前 `1.5.x` 的描述直接当成已验收事实。
...
P1 工作仍属于 `v1.5.11` 范围，但必须由代码级验收测试支撑，并且不能削弱发布 gate。
...
`v1.5.11` 应按下面顺序推进。这个顺序来自研究依据：认知依赖 grounded observation、
...
`v1.5.11` 不应在 P0 工作完成实现、文档和测试覆盖前发布。范围矩阵中的每一项都必须在发布评审时给出明确状态：已实现、部分实现且列出限制、或作为非发布宣称有意延后。静默遗漏即发布阻断。

工作发布审计表见 [`release-audit-1.5.11.md`](release-audit-1.5.11.md)。这张表是规划和实现之间的交接面；如果某一行仍处于部分完成或发布阻断状态，release notes 不能宣称该项已经完成。
