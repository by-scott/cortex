...
- 原生插件加载、可热重载的进程隔离插件工具代理、插件 skills/prompts 和带运行时上下文的工具执行。
...
原生插件使用进程 JSON 边界。插件通过 manifest 声明代理工具，并用 JSON stdin/stdout 协议作为子进程执行，可控制 cwd、环境变量、timeout、输出上限、宿主路径 opt-in，以及 Unix CPU/内存 rlimit。插件文档和脚手架只暴露这一条边界。
