...
- `crates/cortex-types/tests/contracts.rs` checks shared data contracts, serialization, turn transitions, memory ownership, feedback attribution and replay checks, workspace admission and contamination barriers, media attachment governance defaults, exact plugin version targeting, sandbox-enforcement claim rejection, prompt-injection corpus parseability, actor leakage corpus parseability, replay migration corpus parseability, and docs/runtime surface sync for published bilingual README and docs surfaces: event counts, turn-state counts, attention/metacognition/memory-recall wording, plugin-boundary wording, risk-surface guidance, and replay/compaction terminology.
...
- `crates/cortex-kernel/tests/prompt_manager.rs` checks current prompt layout initialization and prompt linting, including checked-update rejection for runtime policy overrides, absent capability references, and unapproved self-edit diffs.
- `crates/cortex-kernel/tests/config_loader.rs` checks current config defaults, actor-binding roundtrips, and runtime-state session-map persistence.
- `crates/cortex-runtime/tests/process_plugin.rs` checks process-isolated plugin registration, manifest version-target and native-ABI rejection, current native-manifest library probing, execution, stderr/non-zero-exit propagation, invalid JSON output rejection, command/working-dir path-boundary validation, host-path opt-in, environment inheritance, timeout/output-limit behavior, backup-directory suppression, governance rejection for unsafe secret access, package capability visibility, and per-tool effect propagation through a shared conformance helper surface; it also checks the forced subprocess effect for process tools that underreport process capability.
...
- `crates/cortex-runtime/src/tests/channel_store.rs` checks current paired-user subscription, policy, and update-offset persistence.
...
- `crates/cortex-runtime/src/tests/rpc_tasks.rs` checks the RPC task surface, including actor-owned task creation, actor-scoped list/get visibility, hidden-task rejection, atomic claim, and checked task state transitions.
- `crates/cortex-runtime/src/tests/rpc_goals.rs` checks the RPC goal surface, including actor-owned goal creation, actor-scoped list/get visibility, hidden-goal rejection, open-goal filtering, and checked goal state transitions.
...
- `crates/cortex-turn/src/agent_pool/delegation.rs` unit tests check delegation contract validation, including explicit allowed-tool filtering, forbidden-action blocking, token/iteration budgets, allowed-evidence declarations, worker prompt rendering, and rejection of broad parent-authority inheritance.
...
Use [Actor Leakage Corpus](actor-leakage-corpus.md) for release-candidate
review of actor-scoped state boundaries. The corpus lives under
`scenarios/actor-leakage/corpus.json` and covers session, memory, task/goal,
retrieval, channel, transport rebind, and audit/operator surfaces. It is an
Eval/Scenario evidence artifact, not sandbox containment, not hostile
multi-tenant hardening, and not proof of complete actor isolation.

Use [Replay Migration Corpus](replay-migration-corpus.md) for release-candidate
review of replay fixture migration and projection stability. The corpus lives
under `scenarios/replay-migration/corpus.json` and references the checked-in
replay fixtures under `crates/cortex-kernel/tests/fixtures/replay/`. It is an
Eval/Scenario evidence artifact, not proof that every historical journal or
database from every previous release migrates without review.
