...
date promise. It is the engineering contract for the `v1.5.6` planning target.

The rule for `v1.5.6` is deliberately narrow: do not replace the mature `v1.4.0`
...
The current planning target is `1.5.6`. It is a patch target on the same 1.5
...
out of scope for the `v1.5.6` release claim.
...
Any `v1.5.6` design or implementation that conflicts with these sources must
...
For `v1.5.6`, this is a boundary condition, not a marketing claim. Cortex should
...
The review that defines `v1.5.6` has twenty-five required areas. The scope
...
`v1.5.6` must continue the strict project gate:
...
area for `v1.5.6`; none of these areas may disappear from implementation,
...
and known limitations are reviewed together. `v1.5.6` must verify each point
...
P1 work remains in scope for `v1.5.6` when it is backed by code-level
...
`v1.5.6` should be implemented in this order. The order follows the research
...
`v1.5.6` should not ship until the P0 work is implemented, documented, and
...
[`release-audit-1.5.6.md`](release-audit-1.5.6.md). That audit table is the
