...
It is the engineering contract for the `v1.6.1` planning target.

The rule for `v1.6.1` is deliberately narrow: keep the harness strong, keep the
...
The current planning target is `1.6.1`. It is the current release target,
...
out of scope for the `v1.6.1` release claim.
...
Any `v1.6.1` design or implementation that conflicts with these sources must
...
For `v1.6.1`, this is a boundary condition, not a marketing claim. Cortex should
...
The review that defines `v1.6.1` has twenty-five required areas. The scope
...
`v1.6.1` must continue the strict project gate:
...
area for `v1.6.1`; none of these areas may disappear from implementation,
...
and known limitations are reviewed together. `v1.6.1` must verify each point
...
P1 work remains in scope for `v1.6.1` when it is backed by code-level
...
`v1.6.1` should be implemented in this order. The order follows the research
...
`v1.6.1` should not ship until the P0 work is implemented, documented, and
...
[`release-audit-1.6.1.md`](release-audit-1.6.1.md). That audit table is the
