...
It is the engineering contract for the `v1.6.8` planning target.

The rule for `v1.6.8` is deliberately narrow: keep the harness strong, keep the
cleanup line closed, and make local OpenAI-compatible/vLLM thinking controls
work correctly under streaming. The release fixes the boundary where providers
can emit Qwen3 reasoning deltas instead of content deltas after thinking is
disabled, and keeps Cortex hidden-thinking filtering incremental rather than
final-response-only.
...
The current planning target is `1.6.8`. It is the current release target,
...
- **Token economy**: provider-facing context keeps stable prefixes stable, and
  status separates context usage, cache read/write, and cumulative spend.
...
out of scope for the `v1.6.8` release claim.
...
Any `v1.6.8` design or implementation that conflicts with these sources must
...
For `v1.6.8`, this is a boundary condition, not a marketing claim. Cortex should
...
The review that defines `v1.6.8` has twenty-five required areas. The scope
...
`v1.6.8` must continue the strict project gate:
...
area for `v1.6.8`; none of these areas may disappear from implementation,
...
and known limitations are reviewed together. `v1.6.8` must verify each point
...
P1 work remains in scope for `v1.6.8` when it is backed by code-level
...
`v1.6.8` should be implemented in this order. The order follows the research
...
`v1.6.8` should not ship until the P0 work is implemented, documented, and
...
[`release-audit-1.6.8.md`](release-audit-1.6.8.md). That audit table is the
