...
The runtime home is treated as a protected root for ordinary tool execution. Tools are blocked from accessing protected instance-state files, symlinked paths are resolved before the check, and process/script tools remain available through the normal permission gate unless their invocation directly targets the protected root. Plugin tools that present prompt, config, session, journal, memory, or runtime-state mutation are blocked from direct mutation; self-evolution plugins should return structured proposals for the checked PromptManager/runtime-command path. This is a governance boundary for prompt/config/state mutation; it is not a replacement for OS-level plugin sandboxing.
