# 1.5.10 Release Audit

This is the working truth table for the `v1.5.10` release target. It is not a
...
surface, the remaining gap, and the `1.5.10` disposition.
...
- **Release blocker**: the area lacks a required runtime contract or acceptance
  proof and must be completed before `1.5.10` ships.

## Audit Table

| Area | Current code evidence | Current verification | `1.5.10` gap | Status |
|------|-----------------------|----------------------|-------------|--------|
...
| Model / Provider Routing | `crates/cortex-types/src/model_routing.rs`, config/provider metadata, and runtime daemon routing expose capability registry, route explanations, fallback, schema-invalid fallback, high-risk escalation, cost/latency/safety/reasoning signals. | `crates/cortex-types/tests/model_routing.rs`, `docs/config.md`, `docs/testing.md`. | Feed route outcomes back into calibration and expose route decisions in release reports/operator traces. | Partial |
...
| Observability | Runtime metrics, HTTP operator dashboard, status surfaces, journal timeline normalization, timeline categories, token/cost/provider/session summaries, and health endpoints exist. | `crates/cortex-runtime/src/tests/http_operator.rs`, `docs/ops.md`, `docs/testing.md`. | Add complete turn timeline views for workspace, retrieval, memory, control, tools, guardrails, risk ledger, and memory change review. | Partial |
...
## 1.5.10 Patch Evidence

`v1.5.10` is the current harness contract and adds focused cleanup plus plugin-tool hardening:
