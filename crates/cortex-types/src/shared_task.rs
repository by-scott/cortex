...
mod tests {
    ...
    fn valid_transitions() {
        assert!(
            SharedTaskStatus::Pending
                .try_transition(SharedTaskStatus::Assigned)
                .is_ok()
        );
        assert!(
            SharedTaskStatus::InProgress
                .try_transition(SharedTaskStatus::Completed)
                .is_ok()
        );
    }
    ...
    fn cancel_from_any() {
        assert!(
            SharedTaskStatus::Pending
                .try_transition(SharedTaskStatus::Cancelled)
                .is_ok()
        );
        assert!(
            SharedTaskStatus::InProgress
    ...
    #[test]
    fn terminal_states() {
        assert!(SharedTaskStatus::Completed.is_terminal());
        assert!(SharedTaskStatus::Failed.is_terminal());
        assert!(SharedTaskStatus::Cancelled.is_terminal());
        assert!(!SharedTaskStatus::Pending.is_terminal());
    }

    #[test]
    fn serde_roundtrip() {
        let task = SharedTask::new("do something");
        let json = serde_json::to_string(&task).unwrap();
        let back: SharedTask = serde_json::from_str(&json).unwrap();
        assert_eq!(back.description, "do something");
