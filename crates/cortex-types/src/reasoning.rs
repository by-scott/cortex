...
mod tests {
    ...
    #[test]
    fn step_confidence_clamped() {
        let s = ReasoningStep::new(0, ReasoningStepType::Premise, "test").with_confidence(1.5);
        assert!((s.confidence - 1.0).abs() < f64::EPSILON);
    }

    #[test]
    fn chain_recalculates() {
        let mut chain = ReasoningChain::new(ReasoningMode::ChainOfThought);
        chain.add_step(ReasoningStep::new(0, ReasoningStepType::Premise, "a").with_confidence(0.8));
        chain.add_step(
            ReasoningStep::new(1, ReasoningStepType::Inference, "b").with_confidence(0.6),
        );
        assert!((chain.overall_confidence - 0.7).abs() < 0.01);
    }
    ...
    fn should_abandon() {
        ...
        for i in 0..3 {
            chain.add_step(
                ReasoningStep::new(i, ReasoningStepType::Inference, "low").with_confidence(0.1),
            );
        }
        assert!(chain.should_abandon());
    }

    #[test]
    fn format_context_not_empty() {
        let chain = ReasoningChain::new(ReasoningMode::ChainOfThought);
        assert!(chain.format_context().contains("CoT"));
    }

    #[test]
    fn serde_roundtrip() {
        let chain = ReasoningChain::new(ReasoningMode::HypothesisTest);
        let json = serde_json::to_string(&chain).unwrap();
        let back: ReasoningChain = serde_json::from_str(&json).unwrap();
        assert_eq!(back.mode, ReasoningMode::HypothesisTest);
    }
}
