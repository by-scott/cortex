...
mod tests {
    ...
    #[test]
    fn link_confidence_clamped() {
        let l = CausalLink::new("a", "b", CausalRelation::Triggers, 1.5);
        assert!((l.confidence - 1.0).abs() < f64::EPSILON);
    }
    ...
    fn chain_from_links() {
        ...
        let chain = CausalChain::from_links(links);
        assert_eq!(chain.root_cause, "a");
        assert_eq!(chain.final_effect, "c");
        assert!((chain.overall_confidence - 0.72).abs() < 0.01);
    }

    #[test]
    fn empty_chain_format() {
        let chain = CausalChain::from_links(vec![]);
        assert_eq!(chain.format(), "(empty causal chain)");
    }

    #[test]
    fn serde_roundtrip() {
        let link = CausalLink::new("x", "y", CausalRelation::Contributes, 0.5);
        let json = serde_json::to_string(&link).unwrap();
        let back: CausalLink = serde_json::from_str(&json).unwrap();
        assert_eq!(back.cause_event, "x");
    }
}
