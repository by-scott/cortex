...
mod tests {
    ...
    #[test]
    fn memory_relation_new() {
        let rel = MemoryRelation::new("a", "b", "relates_to");
    ...
    #[test]
    fn memory_relation_with_metadata() {
        let rel = MemoryRelation::new("a", "b", "depends_on").with_metadata("critical");
        assert_eq!(rel.metadata.as_deref(), Some("critical"));
    }

    #[test]
    fn add_and_query_neighbors() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("m1", "m2", "relates_to"))
            .unwrap();
        g.add_relation(&MemoryRelation::new("m1", "m3", "depends_on"))
            .unwrap();

        let mut neighbors = g.neighbors("m1").unwrap();
        neighbors.sort();
        assert_eq!(neighbors, vec!["m2", "m3"]);
    }
    ...
    fn neighbors_bidirectional() {
        ...
        let n1 = g.neighbors("m1").unwrap();
        assert_eq!(n1, vec!["m2"]);

        let n2 = g.neighbors("m2").unwrap();
        assert_eq!(n2, vec!["m1"]);
    }

    #[test]
    fn neighbors_by_type_filters() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("m1", "m2", "relates_to"))
            .unwrap();
        g.add_relation(&MemoryRelation::new("m1", "m3", "depends_on"))
        ...
        let n = g.neighbors_by_type("m1", "relates_to").unwrap();
        assert_eq!(n, vec!["m2"]);

        let n = g.neighbors_by_type("m1", "depends_on").unwrap();
        assert_eq!(n, vec!["m3"]);
    }
    ...
    fn remove_relation() {
        ...
        g.add_relation(&MemoryRelation::new("m1", "m2", "relates_to"))
            .unwrap();
        assert!(g.remove_relation("m1", "m2", "relates_to").unwrap());
        assert!(g.neighbors("m1").unwrap().is_empty());
    }

    #[test]
    fn remove_nonexistent_returns_false() {
        let g = test_graph();
        assert!(!g.remove_relation("x", "y", "z").unwrap());
    }
    ...
    fn relations_of() {
        ...
        g.add_relation(&MemoryRelation::new("m1", "m2", "relates_to"))
            .unwrap();
        g.add_relation(&MemoryRelation::new("m3", "m1", "depends_on"))
            .unwrap();

        let rels = g.relations_of("m1").unwrap();
        assert_eq!(rels.len(), 2);
    }

    #[test]
    fn reachable_depth_zero() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("m1", "m2", "x"))
            .unwrap();
        let r = g.reachable("m1", 0).unwrap();
        assert!(r.is_empty());
    }

    #[test]
    fn reachable_one_hop() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("m1", "m2", "x"))
            .unwrap();
        g.add_relation(&MemoryRelation::new("m2", "m3", "x"))
            .unwrap();

        let r = g.reachable("m1", 1).unwrap();
        assert!(r.contains("m2"));
        assert!(!r.contains("m3"));
    }

    #[test]
    fn reachable_two_hops() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("m1", "m2", "x"))
            .unwrap();
        g.add_relation(&MemoryRelation::new("m2", "m3", "x"))
            .unwrap();
        g.add_relation(&MemoryRelation::new("m3", "m4", "x"))
            .unwrap();

        let r = g.reachable("m1", 2).unwrap();
        assert!(r.contains("m2"));
        assert!(r.contains("m3"));
        assert!(!r.contains("m4"));
    }
    ...
    fn all_node_ids_returns_all() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("a", "b", "x")).unwrap();
        g.add_relation(&MemoryRelation::new("b", "c", "y")).unwrap();
        let ids = g.all_node_ids().unwrap();
        assert_eq!(ids.len(), 3);
    ...
    #[test]
    fn all_relations_returns_all() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("a", "b", "x")).unwrap();
        g.add_relation(&MemoryRelation::new("b", "c", "y")).unwrap();
        let rels = g.all_relations().unwrap();
        assert_eq!(rels.len(), 2);
    }

    #[test]
    fn degree_map_counts_correctly() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("a", "b", "x")).unwrap();
        g.add_relation(&MemoryRelation::new("a", "c", "y")).unwrap();
        let dm = g.degree_map().unwrap();
        assert_eq!(dm.get("a"), Some(&2));
        assert_eq!(dm.get("b"), Some(&1));
        assert_eq!(dm.get("c"), Some(&1));
    }
    ...
    fn connected_components_two_components() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("a", "b", "x")).unwrap();
        g.add_relation(&MemoryRelation::new("c", "d", "x")).unwrap();
        let nodes: HashSet<String> = ["a", "b", "c", "d"]
            .iter()
            .map(ToString::to_string)
            .collect();
        let comps = g.connected_components(&nodes).unwrap();
    ...
    fn connected_components_single_component() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("a", "b", "x")).unwrap();
        g.add_relation(&MemoryRelation::new("b", "c", "x")).unwrap();
        let nodes: HashSet<String> = ["a", "b", "c"].iter().map(ToString::to_string).collect();
        let comps = g.connected_components(&nodes).unwrap();
    ...
    #[test]
    fn connected_components_with_orphan() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("a", "b", "x")).unwrap();
        let nodes: HashSet<String> = ["a", "b", "c"].iter().map(ToString::to_string).collect();
        let comps = g.connected_components(&nodes).unwrap();
    ...
    #[test]
    fn upsert_duplicate_relation() {
        let g = test_graph();
        g.add_relation(&MemoryRelation::new("m1", "m2", "x"))
            .unwrap();
        g.add_relation(&MemoryRelation::new("m1", "m2", "x").with_metadata("updated"))
            .unwrap();
        let rels = g.relations_of("m1").unwrap();
        assert_eq!(rels.len(), 1);
        assert_eq!(rels[0].metadata.as_deref(), Some("updated"));
    }
}
