...
mod tests {
    ...
    #[test]
    fn cortex_paths_resolve_instance_layout() {
        let paths = CortexPaths::new("/tmp/cortex-base", "work");
        assert_eq!(paths.base_dir(), &PathBuf::from("/tmp/cortex-base"));
        assert_eq!(paths.instance_id(), "work");
        assert_eq!(
            paths.instance_home(),
            PathBuf::from("/tmp/cortex-base/work")
        );
        assert_eq!(
            paths.data_dir(),
            PathBuf::from("/tmp/cortex-base/work/data")
        );
        assert_eq!(
            paths.config_path(),
            PathBuf::from("/tmp/cortex-base/work/config.toml")
        );
        assert_eq!(
            paths.config_defaults_path(),
            PathBuf::from("/tmp/cortex-base/work/config.defaults.toml")
        );
        assert_eq!(
            paths.providers_path(),
            PathBuf::from("/tmp/cortex-base/providers.toml")
        );
        assert_eq!(
            paths.plugins_dir(),
            PathBuf::from("/tmp/cortex-base/plugins")
        );
    }

    #[test]
    fn actor_bindings_store_round_trips_aliases_and_transports() {
        let dir = tempfile::tempdir().unwrap();
        let paths = CortexPaths::new(dir.path(), "default");
        let store = ActorBindingsStore::from_paths(&paths);

        store.set_actor_alias("telegram:1", "user:scott");
        store.set_transport_actor("http", "user:scott");

        assert_eq!(
            store.actor_aliases().get("telegram:1"),
            Some(&"user:scott".to_string())
        );
        assert_eq!(
            store.transport_actors().get("http"),
            Some(&"user:scott".to_string())
        );

        assert!(store.remove_actor_alias("telegram:1"));
        assert!(store.remove_transport_actor("http"));
        assert!(store.actor_aliases().is_empty());
        assert!(store.transport_actors().is_empty());
    }

    #[test]
    fn runtime_state_store_round_trips_session_maps() {
        let dir = tempfile::tempdir().unwrap();
        let paths = CortexPaths::new(dir.path(), "default");
        let store = RuntimeStateStore::from_paths(&paths);
        let mut client_sessions = HashMap::new();
        let mut actor_sessions = HashMap::new();
        client_sessions.insert("http".to_string(), "session-http".to_string());
        actor_sessions.insert("user:scott".to_string(), "session-actor".to_string());
    ...
    fn generated_config_has_header() {
        ...
        assert!(dir.path().join("config.defaults.toml").exists());
        assert!(!dir.path().join("data").join("defaults.toml").exists());
    }
    ...
    fn parse_providers_handles_all_protocols() {
        ...
        assert!(openai.openai_stream_options);
        let zai = registry.get("zai").unwrap();
        assert_eq!(zai.vision_provider, "zai-openai");
        let zai_openai = registry.get("zai-openai").unwrap();
        assert_eq!(zai_openai.image_input_mode, OpenAiImageInputMode::DataUrl);
        assert!(!zai_openai.openai_stream_options);
    }
