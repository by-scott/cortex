...
mod tests {
    ...
    // ── CronTool execute tests ──

    fn test_tool() -> (CronTool, tempfile::TempDir) {
        let dir = tempfile::tempdir().unwrap();
        let tool = CronTool::new(Arc::new(CronQueue::open(dir.path())));
        (tool, dir)
    }

    #[test]
    fn cron_create_action() {
        let (tool, _dir) = test_tool();
        let result = tool
    ...
    fn cron_list_action() {
        let (tool, _dir) = test_tool();
        let result = tool.execute(serde_json::json!({"action": "list"})).unwrap();
        assert!(!result.is_error);
    }

    #[test]
    fn cron_cancel_action() {
        let (tool, _dir) = test_tool();
        ...
        let create_result = tool
            ...
            .unwrap();
        // Extract ID from "Scheduled recurring task <ID> with..."
        let id = create_result
            ...
            .unwrap_or("unknown");
        let result = tool
            .execute(serde_json::json!({"action": "cancel", "task_id": id}))
            .unwrap();
        assert!(!result.is_error);
    ...
    #[test]
    fn cron_cancel_missing_id() {
        let (tool, _dir) = test_tool();
        let result = tool.execute(serde_json::json!({"action": "cancel"}));
        assert!(result.is_err());
    }

    #[test]
    fn cron_unknown_action() {
        let (tool, _dir) = test_tool();
        let result = tool.execute(serde_json::json!({"action": "unknown"}));
        assert!(result.is_err());
    }

    #[test]
    fn cron_missing_action() {
        let (tool, _dir) = test_tool();
        let result = tool.execute(serde_json::json!({}));
        assert!(result.is_err());
    }

    // ── CronQueue persistence tests ──

    #[test]
    fn queue_add_and_list() {
        let dir = tempfile::tempdir().unwrap();
        let queue = CronQueue::open(dir.path());
        let id = queue.add("*/5 * * * *", "hello", true);
    ...
    fn queue_cancel() {
        let dir = tempfile::tempdir().unwrap();
        let queue = CronQueue::open(dir.path());
        let id = queue.add("0 9 * * *", "morning", true);
    ...
    fn queue_cancel_nonexistent() {
        let dir = tempfile::tempdir().unwrap();
        let queue = CronQueue::open(dir.path());
        assert!(!queue.cancel("nonexistent-id"));
    }

    #[test]
    fn queue_collect_due_empty() {
        let dir = tempfile::tempdir().unwrap();
        let queue = CronQueue::open(dir.path());
        assert!(queue.collect_due().is_empty());
    }

    // ── Cron expression matching tests ──

    #[test]
    fn field_wildcard_matches_any() {
    ...
    fn field_range_with_step() {
        assert!(field_matches("0-30/10", 0));
        assert!(field_matches("0-30/10", 10));
        assert!(field_matches("0-30/10", 20));
        assert!(!field_matches("0-30/10", 5));
        assert!(!field_matches("0-30/10", 40));
    }
    ...
    #[test]
    fn is_due_invalid_expr() {
        let now = chrono::Utc::now();
        // Too few fields
        assert!(!is_due("* *", &now, None));
    }

    // ── CronEntry serialization ──

    #[test]
    fn cron_entry_roundtrip() {
        let entry = CronEntry {
            id: "test-id".into(),
            cron_expr: "*/5 * * * *".into(),
            prompt: "hello".into(),
        ...
        let back: CronEntry = serde_json::from_str(&json).unwrap();
        assert_eq!(back.id, "test-id");
        assert_eq!(back.cron_expr, "*/5 * * * *");
        assert!(back.last_run.is_none());
    }

    // ── CronTool structured output ──

    #[test]
    fn create_returns_scheduled_message() {
        let (tool, _dir) = test_tool();
        let result = tool
            .execute(serde_json::json!({
                "action": "create",
            ...
            .unwrap();
        assert!(result.output.contains("Scheduled one-shot task"));
    }
}
