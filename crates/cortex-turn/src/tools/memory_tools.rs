...
/// Components needed for full 5D+Graph memory recall.
///
...
impl MemoryRecallComponents {
    /// Perform full 5D+Graph recall (or BM25 fallback if embeddings unavailable).
        ...
        let results: Vec<&cortex_types::MemoryEntry> =
            match (&self.embedding_client, &self.embedding_store) {
                (Some(ec), Some(cache)) => {
                    ...
                    let graph_scores = MemoryGraph::open(&self.data_dir.join("memory_graph.db"))
                        .ok()
                        .map(|g| {
...
/// Search memories by query using full 5D+Graph hybrid recall.
