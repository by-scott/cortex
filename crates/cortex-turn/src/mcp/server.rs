...
mod tests {
    ...
    fn setup() -> ToolRegistry {
        ...
        crate::tools::register_core_tools_basic(&mut tools);
        tools
    }
    ...
    fn initialize_returns_capabilities() {
        ...
        let req = McpRequest::new(1, "initialize", serde_json::json!({}));
    ...
    fn tools_list_returns_all_tools() {
        let tools = setup();
        let server = McpServer::new(&tools);
        let req = McpRequest::new(2, "tools/list", serde_json::json!({}));
        let resp = server.handle_request(&req);
        assert!(resp.error.is_none());
        let result = resp.result.unwrap();
    ...
    fn unknown_method_returns_error() {
        let tools = setup();
        let server = McpServer::new(&tools);
        let req = McpRequest::new(4, "unknown/method", serde_json::json!({}));
        let resp = server.handle_request(&req);
        assert!(resp.error.is_some());
