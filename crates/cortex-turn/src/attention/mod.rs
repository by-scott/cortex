...
mod tests {
    ...
    #[test]
    fn scheduler_respects_actor_budget_and_explains_skips() {
        let mut scheduler = ChannelScheduler::new().with_actor_budget(1);
        let options = TaskOptions::default().with_actor("user:scott").with_cost(1);
        scheduler.register_with_options(
            AttentionChannel::Foreground,
            "first",
            options.clone(),
            Vec::new,
        );
        scheduler.register_with_options(AttentionChannel::Foreground, "second", options, Vec::new);

        let events = scheduler.tick();
        let foreground_report = events
            .iter()
            ...
            .expect("foreground schedule event should exist");

        assert_eq!(foreground_report.0, 1);
        assert_eq!(foreground_report.1, "user:scott");
        assert_eq!(foreground_report.3, 0);
        assert!(foreground_report.2.contains("skipped by actor budget"));
    }

    #[test]
    fn scheduler_debounces_emergency_and_tracks_maintenance_debt() {
        let mut scheduler = ChannelScheduler::new().with_maintenance_interval(0);
        scheduler.register_with_options(
            AttentionChannel::Emergency,
            "guard",
            TaskOptions::default().with_emergency_debounce_ticks(2),
            Vec::new,
        );
        scheduler.register(AttentionChannel::Maintenance, "compact", Vec::new);

        let first = scheduler.tick();
        assert!(first.iter().any(|event| matches!(
            event,
            Payload::ChannelScheduled { channel, task_count: 1, .. } if channel == "emergency"
        )));
        assert_eq!(scheduler.maintenance_debt(), 1);
        ...
        assert!(second.iter().any(|event| matches!(
            event,
            Payload::ChannelScheduled { channel, task_count: 1, .. } if channel == "maintenance"
        )));
        assert_eq!(scheduler.maintenance_debt(), 0);
    }
}
