...
mod tests {
    ...
    #[test]
    fn signal_precision() {
        let mut t = SignalPrecisionTracker::default();
        t.record_trigger("correction");
        t.record_trigger("correction");
        t.record_improvement("correction");
        assert!((t.precision("correction") - 0.5).abs() < f64::EPSILON);
    }

    #[test]
    fn untriggered_precision_zero() {
        let t = SignalPrecisionTracker::default();
        assert!((t.precision("unknown")).abs() < f64::EPSILON);
    }

    #[test]
    fn update_quality() {
        let mut s = UpdateQualityScorer::default();
        s.record_update(0.5, 0.8);
        s.record_update(0.6, 0.7);
        assert!((s.avg_quality_delta() - 0.2).abs() < f64::EPSILON);
    ...
    fn calibration_snapshot() {
        let mut t = SignalPrecisionTracker::default();
        t.record_trigger("a");
        t.record_improvement("a");
        let s = UpdateQualityScorer::default();
        let snap = take_snapshot(&t, &s);
        assert!((snap.overall_score - 1.0).abs() < f64::EPSILON);
    }
    ...
    fn empty_snapshot() {
        ...
        let s = UpdateQualityScorer::default();
        let snap = take_snapshot(&t, &s);
        assert!((snap.overall_score).abs() < f64::EPSILON);
    }

    #[test]
    fn reconsolidation_effectiveness() {
        let mut r = ReconsolidationTracker::default();
        r.record_marked();
        r.record_marked();
        r.record_updated();
        assert!((r.effectiveness_score() - 0.5).abs() < f64::EPSILON);
    }

    #[test]
    fn reconsolidation_degradation() {
        let mut r = ReconsolidationTracker::default();
        r.record_marked();
        r.record_marked();
        r.record_degraded();
        assert!((r.degradation_rate() - 0.5).abs() < f64::EPSILON);
    }

    #[test]
    fn reconsolidation_snapshot_serializes() {
        let r = ReconsolidationTracker::default();
        let s = r.snapshot();
