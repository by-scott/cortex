...
mod tests {
    ...
    fn make_mem(desc: &str, content: &str) -> MemoryEntry {
        MemoryEntry::new(content, desc, MemoryType::User, MemoryKind::Semantic)
    }

    fn make_mem_with_status(desc: &str, content: &str, status: MemoryStatus) -> MemoryEntry {
        let mut m = make_mem(desc, content);
        m.status = status;
        m
    }

    fn make_old_mem(desc: &str, content: &str, hours_ago: i64) -> MemoryEntry {
        let mut m = make_mem(desc, content);
        m.created_at = Utc::now() - chrono::Duration::hours(hours_ago);
        m
    }

    #[test]
    fn bm25_basic_scoring() {
        assert!(bm25_score("auth login", "the auth system handles login") > 0.0);
        assert!(bm25_score("xyz", "no match here").abs() < f64::EPSILON);
    }

    #[test]
    fn rank_memories_by_relevance() {
        let mems = vec![
            make_mem("css styles", "uses tailwind for styling"),
            make_mem("auth system", "uses JWT for authentication"),
            make_mem("database", "PostgreSQL with auth tokens"),
        ];

        let ranked = rank_memories("auth", &mems, 10);
        assert!(!ranked.is_empty());
        assert!(ranked[0].description.contains("auth") || ranked[0].content.contains("auth"));
    }

    #[test]
    fn rank_returns_top_n() {
        let mems = vec![
            make_mem("a", "matching query here"),
            make_mem("b", "matching query there"),
            make_mem("c", "matching query everywhere"),
        ];
        let ranked = rank_memories("matching", &mems, 2);
    ...
    #[test]
    fn build_memory_context_formats() {
        let m1 = make_mem("user pref", "likes concise");
        let m2 = make_mem("project info", "uses rust");
        let ctx = build_memory_context(&[&m1, &m2]);
        assert!(ctx.contains("user pref"));
        assert!(ctx.contains("uses rust"));
    }
    ...
    #[test]
    fn hybrid_with_embeddings() {
        let mems = vec![
            make_mem("auth system", "uses JWT"),
            make_mem("css styles", "tailwind"),
        ];
        let embs = vec![Some(vec![0.9, 0.1]), Some(vec![0.1, 0.9])];
        let qe = vec![0.8, 0.2];
        let ranked = hybrid_rank(
            "auth",
            Some(&qe),
            ...
            &embs,
            10,
            None::<&HashMap<String, f64>>,
        );
        assert!(!ranked.is_empty());
        assert!(ranked[0].description.contains("auth"));
    }

    #[test]
    fn hybrid_without_embeddings_fallback() {
        let mems = vec![make_mem("auth", "JWT"), make_mem("css", "tailwind")];
        let embs: Vec<Option<Vec<f64>>> = vec![None, None];
        let ranked = hybrid_rank(
            "auth",
            None,
            &mems,
    ...
    fn recency_score_recent() {
        let score = recency_score(Utc::now());
        assert!(
            score > 0.99,
            "recent memory score should be ~1.0, got {score}"
        );
    }

    #[test]
    fn recency_score_old() {
        let old = Utc::now() - chrono::Duration::hours(720);
        let score = recency_score(old);
        assert!(
            score < 0.01,
            "30-day-old memory score should be near 0, got {score}"
        );
    }

    #[test]
    fn status_score_values() {
        assert!((status_score(MemoryStatus::Stabilized) - 1.0).abs() < f64::EPSILON);
        assert!((status_score(MemoryStatus::Materialized) - 0.7).abs() < f64::EPSILON);
        assert!((status_score(MemoryStatus::Captured) - 0.4).abs() < f64::EPSILON);
        assert!((status_score(MemoryStatus::Deprecated) - 0.1).abs() < f64::EPSILON);
    }

    #[test]
    fn access_score_zero() {
        assert!(access_score(0).abs() < f64::EPSILON);
    }

    #[test]
    fn access_score_moderate() {
        let score = access_score(50);
        assert!(
            score > 0.8 && score < 0.9,
            "access_score(50) should be ~0.85, got {score}"
        );
    }

    #[test]
    fn access_score_clamped() {
        let score = access_score(10000);
        assert!(score <= 1.0);
    }

    #[test]
    fn hybrid_newer_memory_ranks_higher() {
        let new_mem = make_mem("auth config", "JWT auth configuration");
        let old_mem = make_old_mem("auth config", "JWT auth configuration", 720);

        let mems = vec![old_mem, new_mem];
        let embs: Vec<Option<Vec<f64>>> = vec![None, None];
        let ranked = hybrid_rank(
            "auth config JWT",
            None,
        ...
        assert_eq!(ranked.len(), 2);
        assert!(
            ranked[0].created_at > ranked[1].created_at,
            "newer memory should rank higher"
        );
    }

    #[test]
    fn hybrid_stabilized_ranks_higher_than_captured() {
        let stabilized =
            make_mem_with_status("auth module", "JWT tokens", MemoryStatus::Stabilized);
        let captured = make_mem_with_status("auth module", "JWT tokens", MemoryStatus::Captured);

        let mems = vec![captured, stabilized];
        let embs: Vec<Option<Vec<f64>>> = vec![None, None];
        let ranked = hybrid_rank(
            "auth JWT",
            None,
        ...
        assert_eq!(ranked.len(), 2);
        assert_eq!(ranked[0].status, MemoryStatus::Stabilized);
    }

    // ── Graph expansion tests ──

    #[test]
    fn graph_expand_adds_neighbors() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("m1", "m3", "relates_to"))
            .unwrap();

        let initial = vec!["m1".into(), "m2".into()];
        let expanded = graph_expand_recall(&initial, &g);
        assert!(expanded.contains(&"m3".to_string()));
        assert!(expanded.contains(&"m1".to_string()));
        assert!(expanded.contains(&"m2".to_string()));
    }

    #[test]
    fn graph_expand_no_duplicates() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("m1", "m3", "x"))
            .unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("m2", "m3", "x"))
            .unwrap();

        let initial = vec!["m1".into(), "m2".into()];
        let expanded = graph_expand_recall(&initial, &g);
        let count = expanded.iter().filter(|id| *id == "m3").count();
        assert_eq!(count, 1);
    }

    #[test]
    fn graph_expand_limits_to_max() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        for i in 0..10 {
            g.add_relation(&cortex_types::MemoryRelation::new(
                "m1",
                format!("extra{i}"),
                "x",
            ))
        ...
        let initial = vec!["m1".into()];
        let expanded = graph_expand_recall(&initial, &g);
        // 1 initial + max 5 expansion = 6
        assert!(expanded.len() <= 6);
    }

    #[test]
    fn graph_expand_empty_graph() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        let initial = vec!["m1".into(), "m2".into()];
        let expanded = graph_expand_recall(&initial, &g);
    ...
    #[test]
    fn graph_expand_skips_initial_ids() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("m1", "m2", "x"))
            .unwrap();
    ...
    // ── Multi-hop graph reasoning tests ──

    #[test]
    fn graph_reasoning_scores_1hop() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("m1", "m2", "x"))
            .unwrap();

        let scores = graph_reasoning_scores(&["m1".into()], &g, 3);
        assert!((scores["m2"] - 0.5).abs() < f64::EPSILON);
    }

    #[test]
    fn graph_reasoning_scores_2hop() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("m1", "m2", "x"))
            .unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("m2", "m3", "x"))
        ...
        let scores = graph_reasoning_scores(&["m1".into()], &g, 3);
        assert!((scores["m2"] - 0.5).abs() < f64::EPSILON);
        assert!((scores["m3"] - 0.25).abs() < f64::EPSILON);
    }

    #[test]
    fn graph_reasoning_scores_respects_max_depth() {
        let g = cortex_kernel::MemoryGraph::in_memory().unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("a", "b", "x"))
            .unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("b", "c", "x"))
            .unwrap();
        g.add_relation(&cortex_types::MemoryRelation::new("c", "d", "x"))
            .unwrap();

        let scores = graph_reasoning_scores(&["a".into()], &g, 2);
        assert!(scores.contains_key("b"));
        assert!(scores.contains_key("c"));
        assert!(!scores.contains_key("d")); // beyond max_depth=2
    }
    ...
    #[test]
    fn hybrid_rank_with_graph_scores() {
        let mut m1 = make_mem("auth system", "JWT tokens");
        let m2 = make_mem("auth system", "JWT tokens");
        m1.id = "boosted".to_string();

        let mems = vec![m2, m1];
        let embs: Vec<Option<Vec<f64>>> = vec![None, None];
        let mut gs = HashMap::new();
        gs.insert("boosted".to_string(), 0.5);

        let ranked = hybrid_rank("auth JWT", None, &mems, &embs, 10, Some(&gs));
        assert_eq!(ranked.len(), 2);
        assert_eq!(ranked[0].id, "boosted");
    }

    // ── Embedding health status tests ──

    #[test]
    fn health_starts_healthy() {
        let health = EmbeddingHealthStatus::new();
        assert!(!health.is_degraded());
    }

    #[test]
    fn health_degrades_after_threshold() {
        let health = EmbeddingHealthStatus::new();
        health.record_failure();
        health.record_failure();
        assert!(!health.is_degraded(), "2 failures < threshold 3");
        health.record_failure();
        assert!(health.is_degraded(), "3 failures = threshold");
    }
    ...
    #[test]
    fn health_cooldown_logic() {
        let health = EmbeddingHealthStatus::new();
        assert!(health.cooldown_elapsed());
        ...
        assert!(health.cooldown_elapsed(), "120s > 60s cooldown");
    }

    #[test]
    fn health_default_is_healthy() {
        let health = EmbeddingHealthStatus::default();
        assert!(!health.is_degraded());
        assert!(health.cooldown_elapsed());
    }
}
