...
mod tests {
    ...
    #[test]
    fn initial_zero() {
        let m = LearningMetrics::new();
        let s = m.snapshot();
        assert_eq!(s.recall_count, 0);
        assert!((s.recall_precision).abs() < f64::EPSILON);
    }

    #[test]
    fn precision_computed() {
        let m = LearningMetrics::new();
        m.record_recall();
        m.record_recall();
        m.record_recall_hit();
        let s = m.snapshot();
        assert!((s.recall_precision - 0.5).abs() < f64::EPSILON);
    }
    ...
    fn reset_clears() {
        let m = LearningMetrics::new();
        m.record_recall();
        m.record_consolidation_merge();
        m.reset();
        let s = m.snapshot();
        assert_eq!(s.recall_count, 0);
        assert_eq!(s.consolidation_merges, 0);
    }

    #[test]
    fn memory_count() {
        let m = LearningMetrics::new();
        m.set_memory_count(42);
        assert_eq!(m.snapshot().memory_count, 42);
    }
