...
mod tests {
    ...
    #[test]
    fn initial_thresholds_unchanged() {
        let at = AdaptiveThresholds::new(3.0, 0.8, 0.5);
        assert_eq!(at.effective_doom_loop_threshold(), 3);
        assert!((at.effective_fatigue_threshold() - 0.8).abs() < f64::EPSILON);
        assert!((at.effective_frame_threshold() - 0.5).abs() < f64::EPSILON);
    }

    #[test]
    fn low_precision_relaxes_threshold() {
        let mut at = AdaptiveThresholds::new(3.0, 0.8, 0.5);
        // Record 10 doom_loop alerts, 8 false positives, 2 confirmed -> precision = 0.2
        for _ in 0..2 {
            at.record_outcome(&AlertKind::DoomLoop, true);
        }
        for _ in 0..8 {
            at.record_outcome(&AlertKind::DoomLoop, false);
        }
        // After 10 records, adjustment happens: threshold should increase
        assert!(at.doom_loop.current > 3.0);
    }

    #[test]
    fn high_precision_tightens_threshold() {
        let mut at = AdaptiveThresholds::new(3.0, 0.8, 0.5);
        // Record 10 doom_loop alerts, 9 confirmed, 1 false -> precision = 0.9
        for _ in 0..9 {
            at.record_outcome(&AlertKind::DoomLoop, true);
        }
        at.record_outcome(&AlertKind::DoomLoop, false);
    ...
    #[test]
    fn threshold_bounded_upper() {
        let mut at = AdaptiveThresholds::new(3.0, 0.8, 0.5);
        // Force many false positives to push threshold up repeatedly
        for _ in 0..100 {
            at.record_outcome(&AlertKind::DoomLoop, false);
        }
        // Should not exceed 3.0 * 1.5 = 4.5
        assert!(at.doom_loop.current <= 4.5 + f64::EPSILON);
    }

    #[test]
    fn threshold_bounded_lower() {
        let mut at = AdaptiveThresholds::new(3.0, 0.8, 0.5);
        // Force many true positives to push threshold down repeatedly
        for _ in 0..100 {
            at.record_outcome(&AlertKind::DoomLoop, true);
        }
        // Should not go below 3.0 * 0.5 = 1.5
        assert!(at.doom_loop.current >= 1.5 - f64::EPSILON);
    }

    #[test]
    fn duration_alerts_ignored() {
        let mut at = AdaptiveThresholds::new(3.0, 0.8, 0.5);
        at.record_outcome(&AlertKind::Duration, true);
        at.record_outcome(&AlertKind::Duration, false);
        // Nothing should change
        assert_eq!(at.effective_doom_loop_threshold(), 3);
    }

    #[test]
    fn no_adjustment_below_interval() {
        let mut at = AdaptiveThresholds::new(3.0, 0.8, 0.5);
        // Only 5 records, below ADJUST_INTERVAL
        for _ in 0..5 {
            at.record_outcome(&AlertKind::Fatigue, false);
        }
