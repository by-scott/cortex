...
mod tests {
    ...
    #[test]
    fn no_doom_loop_initially() {
        let d = DoomLoopDetector::new(3);
        assert!(d.check().is_none());
    }

    #[test]
    fn detect_same_output() {
        let mut d = DoomLoopDetector::new(3);
        for _ in 0..3 {
            d.record_tool_call("read", "file.rs");
            d.record_tool_result("content");
        }
        assert!(d.check().is_some());
    }

    #[test]
    fn no_loop_different_output() {
        let mut d = DoomLoopDetector::new(3);
        for i in 0..3 {
            d.record_tool_call("read", "file.rs");
            d.record_tool_result(&format!("content_{i}"));
        }
        assert!(d.check().is_none());
    }
    ...
    fn different_inputs() {
        let mut d = DoomLoopDetector::new(3);
        for i in 0..3 {
            d.record_tool_call("read", &format!("file{i}.rs"));
            d.record_tool_result("content");
        }
        assert!(d.check().is_none());
    }
