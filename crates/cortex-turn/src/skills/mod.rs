...
mod tests {
    ...
    #[test]
    fn definition_exposes_skill_shape() {
        let reg = SkillRegistry::new();
        reg.register(Box::new(TestSkill::new()));

        let definition = reg.definition("test").expect("skill definition");
        assert_eq!(definition.name, "test");
        assert_eq!(definition.description, "a test skill");
        assert_eq!(definition.execution_mode, ExecutionMode::Inline);
        assert_eq!(definition.metadata.source, SkillSource::System);
    }

    #[test]
    fn render_returns_definition_and_materialized_content() {
        let reg = SkillRegistry::new();
        reg.register(Box::new(TestSkill::new()));

        let rendered = reg.render("test", "alpha").expect("rendered skill");
        assert_eq!(rendered.definition.name, "test");
        let SkillContent::Markdown(content) = rendered.content;
        assert_eq!(content, "test: alpha");
    }
}
