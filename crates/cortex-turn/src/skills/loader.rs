...
mod tests {
    ...
    fn parse_valid() {
        let raw = "---\ndescription: Test\nwhen_to_use: Always\n---\nContent ${ARGS}";
        let skill = parse_skill_md("t", raw, Path::new("/tmp"), &SkillSource::System).unwrap();
        assert_eq!(skill.desc, "Test");
        let SkillContent::Markdown(c) = skill.content("x");
        assert!(c.contains('x'));
    }

    #[test]
    fn parse_no_frontmatter() {
        assert!(parse_skill_md("t", "no fm", Path::new("/tmp"), &SkillSource::System).is_none());
    }
    ...
    fn load_from_dir() {
        ...
        let sd = dir.path().join("my-skill");
        fs::create_dir_all(&sd).unwrap();
        fs::write(sd.join("SKILL.md"), "---\ndescription: D\n---\nC").unwrap();
        let skills = load_skills(dir.path(), &SkillSource::Instance);
