...
mod tests {
    ...
    fn mock_tools() -> Arc<ToolRegistry> {
        let mut reg = ToolRegistry::new();
        crate::tools::register_core_tools_basic(&mut reg);
        Arc::new(reg)
    }

    fn mock_gate() -> Arc<dyn PermissionGate> {
    ...
    fn test_task_delegation_builder() {
        let task = TaskDelegation::new("search", "find docs").with_mode("full");
        assert_eq!(task.name, "search");
        assert_eq!(task.prompt, "find docs");
        assert_eq!(task.mode, "full");
    }

    #[test]
    fn test_task_delegation_default_mode() {
        let task = TaskDelegation::new("task1", "do something");
        assert_eq!(task.mode, "readonly");
    }

    #[test]
    fn test_task_delegation_with_team_name() {
        let task = TaskDelegation::new("task1", "do something")
            .with_mode("teammate")
            .with_team_name("review-team");
        assert_eq!(task.team_name, Some("review-team".into()));
    }

    #[tokio::test]
    async fn test_delegate_tasks_with_llm() {
        let mock = Arc::new(MockLlmClient::new());
        mock.push_text("Analysis of task-a complete.");
        mock.push_text("Analysis of task-b complete.");

        let tasks = vec![
            TaskDelegation::new("task-a", "analyze module A"),
            TaskDelegation::new("task-b", "analyze module B"),
        ];

        let results = delegate_tasks(
            tasks,
            mock,
        ...
        for r in &results {
            assert!(r.success, "task {} should succeed: {}", r.name, r.output);
            assert!(r.output.contains("Analysis of"));
        }
    }

    #[tokio::test]
    async fn test_delegate_empty_tasks() {
        let mock = Arc::new(MockLlmClient::new());
        let results = delegate_tasks(
            vec![],
            mock,
    ...
    #[tokio::test]
    async fn test_delegate_llm_error() {
        // MockLlmClient with no responses will return error
        let mock = Arc::new(MockLlmClient::new());

        let tasks = vec![TaskDelegation::new("failing-task", "do something")];
        ...
        assert!(results[0].output.contains("LLM error"));
    }

    #[tokio::test]
    async fn test_delegate_full_mode_tool_loop() {
        use crate::llm::{LlmResponse as Resp, LlmToolCall as TC, Usage as U};
        let mock = Arc::new(MockLlmClient::new());

        // First: tool call for read
        ...
        // Second: final text response after tool result
        mock.push_response(Resp {
            text: Some("File contains: hello world".into()),
            tool_calls: vec![],
            usage: U {
                input_tokens: 150,
                output_tokens: 30,
        ...
        let tasks = vec![TaskDelegation::new("reader", "read the file").with_mode("full")];

        // Use full tool set for full mode
        let mut full_reg = ToolRegistry::new();
        crate::tools::register_core_tools_basic(&mut full_reg);
        let full_tools = Arc::new(full_reg);
        ...
        assert_eq!(results[0].input_tokens, 250);
        assert_eq!(results[0].output_tokens, 80);
    }

    #[tokio::test]
    async fn test_delegate_max_iterations() {
        use crate::llm::{LlmResponse as Resp, LlmToolCall as TC, Usage as U};
        let mock = Arc::new(MockLlmClient::new());

        // Push 3 tool-call responses (config has max_iterations=2)
        for i in 0..3 {
            mock.push_response(Resp {
                text: None,
                tool_calls: vec![TC {
        ...
        let tasks = vec![TaskDelegation::new("looper", "keep searching").with_mode("full")];
        ...
        let results = delegate_tasks(tasks, mock, config, full_tools, mock_gate()).await;

        assert_eq!(results.len(), 1);
        assert!(results[0].output.contains("[max iterations reached]"));
    }
    ...
    fn test_aggregate_all_success() {
        ...
        assert!(summary.contains("2/2 tasks succeeded"));
        assert!(summary.contains("[OK] t1"));
        assert!(summary.contains("[OK] t2"));
        assert!(summary.contains("300in/110out"));
    }
    ...
    fn test_aggregate_mixed_results() {
        ...
        assert!(summary.contains("[OK] ok-task"));
        assert!(summary.contains("[FAILED] fail-task"));
    }

    #[test]
    fn test_aggregate_empty() {
        let summary = aggregate_results(&[]);
        assert_eq!(summary, "No tasks delegated.");
    }

    #[test]
    fn test_parse_usage_suffix() {
        let (output, inp, out) = parse_usage_suffix("Hello world\n__USAGE__:100:50");
    ...
    #[test]
    fn test_parse_usage_suffix_no_suffix() {
        let (output, inp, out) = parse_usage_suffix("Hello world");
        assert_eq!(output, "Hello world");
        assert_eq!(inp, 0);
        assert_eq!(out, 0);
    ...
    #[test]
    fn test_worker_system_prompt_readonly() {
        let prompt = worker_system_prompt("readonly", None, None);
        assert!(prompt.is_some());
        assert!(prompt.unwrap().contains("Read-only sub-agent"));
    }

    #[test]
    fn test_worker_system_prompt_full() {
        let prompt = worker_system_prompt("full", None, None);
        assert!(prompt.is_some());
        assert!(prompt.unwrap().contains("Full-access sub-agent"));
    }

    #[test]
    fn test_worker_system_prompt_teammate() {
        let prompt = worker_system_prompt("teammate", Some("review-team"), None);
        assert!(prompt.is_some());
        assert!(prompt.unwrap().contains("review-team"));
    }
}
