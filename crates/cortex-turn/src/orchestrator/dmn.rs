...
pub async fn run_post_tpn_phase(mut ctx: PostTpnContext<'_>) -> Result<TurnResult, TurnError> {
    ...
    // De-sloppify: emit quality check signal when modifying tools were used
    ...
    if !modifying_tools.is_empty() {
        ...
        // Quality -> Skill bridge: activate skills matching QualityCheckSuggested
...
fn apply_working_memory_pressure(ctx: &mut PressureContext<'_>, action: &PressureAction) {
    match action {
        ...
        PressureAction::CompressHistory | PressureAction::TrimHistory { .. } => {}
    }
}
...
pub async fn apply_compress_history(ctx: &mut PressureContext<'_>) {
    if let Some(tpl) = ctx.compress_template {
        ...
        apply_trim_history(ctx, crate::context::DEFAULT_KEEP_RECENT_ROUNDS);
    }
}

fn apply_trim_history(ctx: &mut PressureContext<'_>, keep_rounds: usize) {
    let original_tokens: usize = ctx
        .history
        .iter()
        .map(|m| estimate_tokens(&m.text_content()))
        .sum();
    let removed = crate::context::trim_sliding_window(ctx.history, keep_rounds);
    if removed > 0 {
        let compressed_tokens: usize = ctx
            .history
            .iter()
...
pub async fn handle_context_pressure(ctx: &mut PressureContext<'_>) {
    ...
    let level = PressureLevel::from_occupancy(occupancy, &[0.60, 0.75, 0.85, 0.95]);
    ...
    for action in &resp.actions {
        match action {
            ...
            PressureAction::TrimHistory { keep_rounds } => {
                apply_trim_history(ctx, *keep_rounds);
            }
        }
    }
...
mod tests {
    ...
    fn reflect_high_quality_chain() {
        use cortex_types::{ReasoningChain, ReasoningMode, ReasoningStep, ReasoningStepType};
        let mut chain = ReasoningChain::new(ReasoningMode::ChainOfThought);
        chain.add_step(
            ReasoningStep::new(0, ReasoningStepType::Premise, "Given A").with_confidence(0.7),
        );
        chain.add_step(
            ReasoningStep::new(1, ReasoningStepType::Inference, "Therefore B").with_confidence(0.8),
        );
        chain.add_step(
            ReasoningStep::new(2, ReasoningStepType::Conclusion, "So C").with_confidence(0.9),
        );
        chain.finalize("So C", 0.9);
        ...
        match ev {
            Payload::ReasoningReflection {
                ...
                assert!(quality_score > 0.5, "high quality chain: {quality_score}");
                assert!(weaknesses.is_empty(), "no weaknesses: {weaknesses:?}");
            }
            _ => panic!("expected ReasoningReflection"),
        }
    }

    #[test]
    fn reflect_declining_confidence_chain() {
        use cortex_types::{ReasoningChain, ReasoningMode, ReasoningStep, ReasoningStepType};
        let mut chain = ReasoningChain::new(ReasoningMode::ChainOfThought);
        chain
            .add_step(ReasoningStep::new(0, ReasoningStepType::Premise, "A").with_confidence(0.95));
        chain.add_step(
            ReasoningStep::new(1, ReasoningStepType::Inference, "B").with_confidence(0.2),
        );
        chain.add_step(
            ReasoningStep::new(2, ReasoningStepType::Inference, "C").with_confidence(0.1),
        );
        // No conclusion step -- severe decline from 0.95 -> 0.2 -> 0.1
        ...
        match ev {
            Payload::ReasoningReflection {
                ...
                assert!(quality_score < 0.5, "low quality expected: {quality_score}");
                assert!(
                    weaknesses.contains(&"declining_confidence".to_string()),
                    "should detect declining confidence: {weaknesses:?}"
                );
                assert!(
                    weaknesses.contains(&"no_conclusion".to_string()),
                    "should detect no conclusion: {weaknesses:?}"
                );
            }
            _ => panic!("expected ReasoningReflection"),
        }
    ...
    #[test]
    fn reflect_no_conclusion_chain() {
        use cortex_types::{ReasoningChain, ReasoningMode, ReasoningStep, ReasoningStepType};
        let mut chain = ReasoningChain::new(ReasoningMode::ChainOfThought);
        chain.add_step(ReasoningStep::new(0, ReasoningStepType::Premise, "X").with_confidence(0.8));

        let ev = reflect_on_reasoning(&chain);
        match ev {
            Payload::ReasoningReflection { weaknesses, .. } => {
                assert!(
                    weaknesses.contains(&"no_conclusion".to_string()),
                    "should flag missing conclusion: {weaknesses:?}"
                );
            }
            _ => panic!("expected ReasoningReflection"),
        }
    ...
    // ── Causal retrospect tests ─────────────────────────────────

    #[test]
    fn causal_retrospect_identifies_known_patterns() {
        let turn_id = TurnId::new();
        let corr_id = CorrelationId::new();

        // Construct events matching known causal pattern:
        // ContextPressureObserved -> ContextCompacted
        let events = vec![
            Payload::TurnStarted,
        ...
        let stored = events_to_stored(&events, turn_id, corr_id);
        let analyzer = CausalAnalyzer::new();
        let links = analyzer.analyze_heuristic(&stored);
        let chains = analyzer.build_chains(&links);
        ...
        assert!(
            !chains.is_empty(),
            "should find causal chain for pressure->compact"
        );
        assert_eq!(chains[0].root_cause, "ContextPressureObserved");
        assert_eq!(chains[0].final_effect, "ContextCompacted");
    }

    #[test]
    fn causal_retrospect_empty_on_few_events() {
        let turn_id = TurnId::new();
        let corr_id = CorrelationId::new();

        let events = vec![Payload::TurnStarted, Payload::TurnCompleted];
        ...
        assert!(chains.is_empty(), "no causal patterns in minimal events");
    }
}
