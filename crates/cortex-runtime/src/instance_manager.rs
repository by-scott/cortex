...
/// Files that indicate a directory is an instance (not just a random subdir).
const INSTANCE_MARKERS: &[&str] = &["config.toml", "data", "prompts", "memory"];
        ...
        for entry in entries.flatten() {
                ...
                && let Some(name) = path.file_name().and_then(|n| n.to_str())
            {
                result.push(InstanceInfo {
                    id: name.to_string(),
                    home_path: path,
                    config_exists: entry.path().join("config.toml").exists(),
                });
            }
        }
