...
mod tests {
    ...
    #[test]
    fn parse_classifies_control_and_session_commands() {
        let registry = DefaultCommandRegistry::new();
        let status = registry.parse("/status");
        let session = registry.parse("/session list");
        let switch = registry.parse("/session switch abc123");

        assert_eq!(status.scope, CommandScope::Control);
        assert_eq!(status.kind, CommandKind::Control(ControlCommand::Status));
        assert_eq!(session.scope, CommandScope::Session);
        assert_eq!(session.kind, CommandKind::Session(SessionCommand::List));
        assert_eq!(session.args, "list");
        assert_eq!(
            switch.kind,
            CommandKind::Session(SessionCommand::Switch { target: "abc123" })
        );
    }

    #[test]
    fn classify_groups_control_builtin_and_unknown() {
        let registry = DefaultCommandRegistry::new();

        assert_eq!(
            registry.classify("/status"),
            CommandInvocation::Control(ControlCommand::Status)
        );
        assert!(matches!(
            registry.classify("/session list"),
            CommandInvocation::Builtin(ParsedCommand {
                kind: CommandKind::Session(SessionCommand::List),
                ..
            })
        ));
        assert!(matches!(
            registry.classify("/madeup"),
            CommandInvocation::Unknown(_)
        ));
    }
