...
use cortex_turn::orchestrator::{ToolProgress, TurnConfig, TurnContext, run_turn};
use cortex_turn::risk::PermissionGate;
use cortex_turn::tools::ToolRegistry;
use cortex_types::config::CortexConfig;
use cortex_types::{CorrelationId, Event, Message, Payload, PromptLayer, ResumePacket, TurnId};
...
pub struct TurnExecutorConfig<'a> {
    ...
    /// External cancellation flag — shared with the daemon for `/stop` support.
    pub cancel_flag: Option<std::sync::Arc<std::sync::atomic::AtomicBool>>,
    /// Shared inbox for mid-turn message injection.
...
impl<'a> TurnExecutor<'a> {
    ...
    pub fn execute(
        ...
        match result {
            Ok(turn_result) => {
                ...
                let response_text = turn_result.response_text.or_else(|| {
                    history
                        .iter()
                        .rev()
    ...
    fn run_turn_blocking(
        ...
        // When images are present, prefer the vision-capable LLM so that
        // text-only primary models (e.g. glm-5.1) don't receive unsupported
        // image content blocks.
        let effective_llm: &dyn LlmClient = if images.is_empty() {
            self.cfg.llm
        } else {
            self.cfg.vision_llm.unwrap_or(self.cfg.llm)
        };
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                run_turn(TurnContext {
                    ...
                    on_tool_progress: callbacks.on_tool_progress,
                    images,
                    compress_template,
                    ...
                    cancel_flag: self.cfg.cancel_flag.clone(),
                    message_inbox: self.cfg.message_inbox.clone(),
                })
                .await
            })
        })
    ...
    fn persist_entity_relations(
        ...
        if let Ok(graph) = MemoryGraph::open(&self.cfg.data_dir.join("memory_graph.db")) {
    ...
    fn build_system_prompt(&self, input: &str) -> Option<String> {
        ...
        let relevant = match (self.cfg.embedding_client, self.cfg.embedding_store) {
            (Some(ec), Some(cache)) => {
                ...
                let graph_scores = MemoryGraph::open(&self.cfg.data_dir.join("memory_graph.db"))
                    .ok()
                    .map(|g| {
