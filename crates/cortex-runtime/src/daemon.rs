...
const DEFAULT_DASHBOARD_TIMELINE_LIMIT: usize = 80;
const MAX_DASHBOARD_TIMELINE_LIMIT: usize = 500;
...
impl DaemonState {
    ...
    pub(crate) fn operator_dashboard(&self, requested_limit: usize) -> serde_json::Value {
        let limit = dashboard_timeline_limit(requested_limit);
        let events = self.journal.recent_events(limit).unwrap_or_default();
        let timeline: Vec<serde_json::Value> = events.iter().map(timeline_entry).collect();
        let metrics = self.metrics.snapshot();
        ...
        serde_json::json!({
            ...
            "provider": {
                ...
                "profiles": model_profiles_json(&registry.profiles),
            },
...
fn dashboard_timeline_limit(requested: usize) -> usize {
    if requested == 0 {
        DEFAULT_DASHBOARD_TIMELINE_LIMIT
    } else {
        requested.min(MAX_DASHBOARD_TIMELINE_LIMIT)
    }
}

fn model_profiles_json(profiles: &[cortex_types::ModelProfile]) -> Vec<serde_json::Value> {
    profiles
        .iter()
        .map(|profile| {
            serde_json::json!({
...
fn timeline_counts(events: &[cortex_kernel::StoredEvent]) -> BTreeMap<&'static str, u64> {
...
fn timeline_entry(event: &cortex_kernel::StoredEvent) -> serde_json::Value {
...
impl DaemonServer {
    ...
    fn build_http_router(http_state: HttpState) -> Router<()> {
        ...
        let protected = Router::new()
            ...
            .route("/api/operator/dashboard", get(handle_operator_dashboard))
...
struct OperatorDashboardQuery {
    limit: Option<usize>,
}

async fn handle_operator_dashboard(
    State(state): State<HttpState>,
