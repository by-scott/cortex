...
mod tests {
    ...
    #[test]
    fn broadcast_done_plain_text_keeps_text_literal() {
        let event = BroadcastEvent::done(
            "hello [literal media path] world".into(),
            vec![cortex_types::ResponsePart::Text {
                text: "hello [literal media path] world".into(),
                format: cortex_types::TextFormat::Markdown,
            }],
        );
        assert_eq!(event.plain_text(), "hello [literal media path] world");
    }
}
