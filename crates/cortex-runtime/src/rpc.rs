...
mod tests {
    ...
    #[test]
    fn command_dispatch_accepts_optional_session_id_param() {
        let json = r#"{"jsonrpc":"2.0","method":"command/dispatch","id":1,"params":{"command":"/status","session_id":"abc123"}}"#;
        let req = parse_request(json).unwrap();
        assert_eq!(req.params["command"], "/status");
        assert_eq!(req.params["session_id"], "abc123");
    }

    #[test]
    fn success_response_can_carry_structured_assistant_output() {
        ...
        assert!(json.contains("\"response_format\":\"markdown\""));
        assert!(json.contains("\"response_parts\""));
    }
