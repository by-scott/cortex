...
mod tests {
    ...
    #[test]
    fn idle_tick_returns_deprecate_only() {
        let config = default_config();
        let mut engine = HeartbeatEngine::new(&config);
        let state = HeartbeatState::new();

        let actions = engine.tick(&state);
        // With default thresholds, only DeprecateExpired fires (always checked)
        assert!(
            actions.contains(&HeartbeatAction::DeprecateExpired),
            "actions: {actions:?}"
        );
        // No LLM actions when no corrections and not idle long enough
        assert!(
            !actions.iter().any(HeartbeatAction::requires_llm),
            "no LLM actions expected: {actions:?}"
        );
    }

    #[test]
    fn foreground_busy_returns_empty() {
        let config = default_config();
        let mut engine = HeartbeatEngine::new(&config);
        let state = HeartbeatState::new();
        state.foreground_busy.store(true, Ordering::Relaxed);
    ...
    #[test]
    fn consolidation_triggers_at_threshold() {
        let config = default_config();
        let mut engine = HeartbeatEngine::new(&config);
        let state = HeartbeatState::new();
        state.pending_consolidation.store(5, Ordering::Relaxed);

        let actions = engine.tick(&state);
        assert!(actions.contains(&HeartbeatAction::ConsolidateMemories));
    }

    #[test]
    fn consolidation_below_threshold_skips() {
        let config = default_config();
        let mut engine = HeartbeatEngine::new(&config);
        let state = HeartbeatState::new();
        state.pending_consolidation.store(4, Ordering::Relaxed);
    ...
    #[test]
    fn llm_rate_limit_respected() {
        let mut config = default_config();
        config.limits.max_llm_calls_per_hour = 1;
        let mut engine = HeartbeatEngine::new(&config);
        let state = HeartbeatState::new();
        state.correction_count.store(10, Ordering::Relaxed);

        // First tick: SelfUpdate should appear
        let actions = engine.tick(&state);
        assert!(actions.contains(&HeartbeatAction::SelfUpdate));

        // Simulate LLM call
        state.record_llm_call();

        // Second tick: SelfUpdate should NOT appear (rate limited)
        let actions = engine.tick(&state);
        assert!(!actions.contains(&HeartbeatAction::SelfUpdate));
    }

    #[test]
    fn actions_sorted_by_salience() {
        let config = default_config();
        let mut engine = HeartbeatEngine::new(&config);
        let state = HeartbeatState::new();
        state.pending_consolidation.store(100, Ordering::Relaxed);
        state.pending_embeddings.store(5, Ordering::Relaxed);
