...
impl IntrospectPaths {
    #[must_use]
    pub fn for_home(home: &Path) -> Self {
        Self {
            paths: cortex_kernel::CortexPaths::from_instance_home(home),
        }
    }

    #[must_use]
    pub fn cortex_db_path(&self) -> PathBuf {
        self.paths.cortex_db_path()
    }
...
impl Tool for AuditTool {
    ...
    fn execute(&self, input: serde_json::Value) -> Result<ToolResult, ToolError> {
        ...
        let journal = cortex_kernel::Journal::open(self.paths.cortex_db_path())
...
impl Tool for PromptInspectTool {
    ...
    fn execute(&self, input: serde_json::Value) -> Result<ToolResult, ToolError> {
        let layer = input
            ...
            .and_then(serde_json::Value::as_str)
            .unwrap_or("all");
        let prompts_dir = self.paths.prompts_dir();
...
impl Tool for MemoryGraphTool {
    ...
    fn execute(&self, input: serde_json::Value) -> Result<ToolResult, ToolError> {
        ...
        let db_path = self.paths.memory_graph_path();
...
pub fn register_introspect_tools(
    ...
    let paths = IntrospectPaths::for_home(home);
