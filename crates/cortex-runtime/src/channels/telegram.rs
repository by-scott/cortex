...
#[cfg(test)]
mod tests {
    use super::{TelegramChannel, prefer_final_text};

    #[test]
    fn md_to_html_renders_common_markdown() {
        let html = TelegramChannel::md_to_html(
    ...
    fn md_to_html_escapes_raw_html() {
        let html = TelegramChannel::md_to_html("<b>x</b> & text");
        assert!(html.contains("&lt;b&gt;x&lt;/b&gt;"));
        assert!(html.contains("&amp; text"));
    }

    #[test]
    fn md_to_html_downgrades_tables_into_readable_rows() {
        let html = TelegramChannel::md_to_html(
            "| Col A | Col B |\n| --- | --- |\n| left | right |\n| up | down |",
        );
        assert!(html.contains("Col A"));
        assert!(html.contains("Col B"));
    ...
    #[test]
    fn split_text_for_bubble_prefers_paragraph_boundary() {
        let paragraph =
            "## Section\n\nThis paragraph contains **formatted** content and several words.\n";
        let text = format!("{paragraph}\n{paragraph}\nTrailing.");
        let limit = TelegramChannel::rendered_len(&(paragraph.to_string() + "\nTail"));
        let (prefix, suffix) =
            TelegramChannel::split_text_for_bubble(&text, limit).expect("split expected");

        assert_eq!(prefix, paragraph.trim_end());
        assert!(suffix.starts_with("## Section"));
        assert!(TelegramChannel::rendered_len(&prefix) <= limit);
    }

    #[test]
    fn split_text_for_bubble_rebalances_fenced_code_blocks() {
        let code =
            "```rust\nfn main() {\n    println!(\"hello\");\n    println!(\"world\");\n}\n```";
        let limit =
            TelegramChannel::rendered_len("```rust\nfn main() {\n    println!(\"hello\");\n```");
        let (prefix, suffix) =
            TelegramChannel::split_text_for_bubble(code, limit).expect("split expected");

        assert!(prefix.ends_with("```"));
        assert!(suffix.starts_with("```\n"));
        assert!(TelegramChannel::rendered_len(&prefix) <= limit);
        assert!(prefix.contains("hello"));
        assert!(suffix.contains("world"));
    }

    #[test]
    fn prefer_final_text_always_replaces_stream_draft() {
        let mut buf = "1".to_string();
        prefer_final_text(&mut buf, "1. first\n2. second");
        assert_eq!(buf, "1. first\n2. second");
    }
}
