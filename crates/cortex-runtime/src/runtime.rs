...
impl Drop for CortexRuntime {
    fn drop(&mut self) {
        if self.plugin_libraries.is_empty() {
            return;
        }
        ...
        tracing::debug!(
            count,
            "kept native plugin libraries mapped until process exit"
        );
    }
}
...
impl CortexRuntime {
    ...
    pub async fn new(base: &Path, instance_home: &Path) -> Result<Self, RuntimeError> {
        ...
        let (providers, resolved_provider) = load_providers_for_paths(&paths)
            .map_err(|e| RuntimeError(format!("load providers: {e}")))?;
        let config = load_config_for_paths(&paths, resolved_provider.as_deref(), &providers);

        let db_path = paths.cortex_db_path();
        ...
        let mut primary_endpoint = ResolvedEndpoint::resolve_primary(&config.api, &providers)
            .map_err(|e| RuntimeError(format!("resolve LLM endpoint: {e}")))?;
        primary_endpoint.capability_cache_path = paths
            .model_info_dir()
        ...
        let max_output_tokens = fetch_model_max_output(&paths, &primary_endpoint, &config).await;

        let tools = init_tools();

        let (embedding_client, embedding_store) = init_embedding(&config, &providers, &paths);
...
fn init_embedding(
    ...
    let embedding_store = EmbeddingStore::open(&paths.embedding_store_path()).ok();
...
async fn fetch_model_max_output(
    ...
    let mut store = ModelInfoStore::open(&paths.model_info_dir());
