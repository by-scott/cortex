...
impl<'a> SessionManager<'a> {
    ...
    pub fn create_session(&self) -> (SessionId, SessionMetadata) {
        self.create_session_inner(SessionId::new())
    }
    ...
    pub fn create_session_with_id(&self, id_str: &str) -> (SessionId, SessionMetadata) {
        id_str.parse().map_or_else(
            |_| {
                let (sid, mut meta) = self.create_session_inner(SessionId::new());
                meta.name = Some(id_str.to_string());
                let _ = self.session_store.save(&meta);
                (sid, meta)
            },
            |sid| self.create_session_inner(sid),
        )
    }

    fn create_session_inner(&self, session_id: SessionId) -> (SessionId, SessionMetadata) {
        let event = Event::new(
            TurnId::new(),
            CorrelationId::new(),
        ...
        let meta = SessionMetadata::new(session_id, offset);
        let _ = self.session_store.save(&meta);
        (session_id, meta)
    }
    ...
    pub fn resume_session(
        ...
        match matched.len() {
            ...
            1 => {
                ...
                // Create new session
                let (new_id, new_meta) = self.create_session();
...
mod tests {
    ...
    fn create_and_list_session() {
        ...
        let mgr = SessionManager::new(&journal, &session_store);

        let (sid, meta) = mgr.create_session();
    ...
    #[test]
    fn end_session_updates_metadata() {
        let tmp = tempfile::tempdir().unwrap();
        let db_path = tmp.path().join("test.db");
        let journal = Journal::open(&db_path).unwrap();
        let session_store = SessionStore::open(&tmp.path().join("sessions")).unwrap();
        ...
        let mgr = SessionManager::new(&journal, &session_store);
        let (_sid, mut meta) = mgr.create_session();

        mgr.end_session(&mut meta, 5);
    ...
    fn resume_not_found() {
        ...
        let result = mgr.resume_session("nonexistent", &mut meta, 0);
        assert!(result.is_err());
        match result.unwrap_err() {
            SessionError::NotFound(prefix) => assert_eq!(prefix, "nonexistent"),
            SessionError::Ambiguous(ids) => panic!("expected NotFound, got Ambiguous: {ids:?}"),
        }
    }

    #[test]
    fn resume_creates_new_session() {
        let tmp = tempfile::tempdir().unwrap();
        let db_path = tmp.path().join("test.db");
        let journal = Journal::open(&db_path).unwrap();
        let session_store = SessionStore::open(&tmp.path().join("sessions")).unwrap();
        ...
        let (first_id, mut first_meta) = mgr.create_session();
        mgr.end_session(&mut first_meta, 2);

        // Create second (current) session
        let (_current_id, mut current_meta) = mgr.create_session();

        // Resume first session — use full ID to avoid ambiguity with UUIDs
        let full_id = first_id.to_string();
        let resumed = mgr.resume_session(&full_id, &mut current_meta, 0).unwrap();
        assert_eq!(resumed.restored_from, &full_id[..8]);
        assert!(!resumed.new_session_id.to_string().is_empty());

        // Should now have 3 sessions (first + second-ended + new)
