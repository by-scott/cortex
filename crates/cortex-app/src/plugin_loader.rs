...
fn process_plugin_dir(
    ...
    let manifest_path = sub.join("manifest.toml");
    if !manifest_path.is_file() {
        tracing::debug!(dir = %sub.display(), "no manifest.toml, skipping");
        return empty;
    }
    ...
    let skill_dir = if manifest.capabilities.skills() {
        let skills_path = sub.join("skills");
    ...
    let prompt_dir = if manifest.capabilities.prompts() {
        let prompts_path = sub.join("prompts");
...
mod tests {
    ...
    fn skills_and_prompts_dirs_collected() {
        ...
        std::fs::create_dir_all(pd.join("skills")).unwrap();
        std::fs::create_dir_all(pd.join("prompts")).unwrap();
        std::fs::write(
            pd.join("manifest.toml"),
