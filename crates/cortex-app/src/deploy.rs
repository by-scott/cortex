...
mod tests {
    ...
    #[test]
    fn test_parse_nested_subcommand_skips_id_flag_pair() {
        let args = vec![
            "channel".to_string(),
            "--id".to_string(),
            "work".to_string(),
        ...
        let invocation = parse_nested_subcommand(&args, "channel");
        assert_eq!(invocation.subcommand, Some("approve"));
        assert_eq!(
            invocation.remaining,
            &["telegram".to_string(), "123".to_string()]
        );
    }

    #[test]
    fn test_parse_nested_subcommand_skips_standalone_flags() {
        let args = vec![
            "browser".to_string(),
            "--system".to_string(),
            "enable".to_string(),
        ];
        let invocation = parse_nested_subcommand(&args, "browser");
        assert_eq!(invocation.subcommand, Some("enable"));
        assert!(invocation.remaining.is_empty());
    }

    #[test]
    fn test_parse_channel_subcommand_known_and_unknown() {
        assert_eq!(
            parse_channel_subcommand(Some("approve")),
            Some(ChannelSubcommand::Approve)
        );
        assert_eq!(parse_channel_subcommand(Some("bogus")), None);
        assert_eq!(parse_channel_subcommand(None), None);
    }

    #[test]
    fn test_parse_binding_action_known_and_unknown() {
        assert_eq!(
            parse_binding_action(Some("list")),
            Some(BindingAction::List)
        );
        assert_eq!(parse_binding_action(Some("set")), Some(BindingAction::Set));
        assert_eq!(
            parse_binding_action(Some("unset")),
            Some(BindingAction::Unset)
        );
        assert_eq!(parse_binding_action(Some("bogus")), None);
        assert_eq!(parse_binding_action(None), None);
    }

    #[test]
    fn test_parse_node_subcommand_defaults_to_status() {
        assert_eq!(parse_node_subcommand(None), Ok(NodeSubcommand::Status));
        assert_eq!(
            parse_node_subcommand(Some("setup")),
            Ok(NodeSubcommand::Setup)
        );
        assert!(parse_node_subcommand(Some("bogus")).is_err());
    }

    #[test]
    fn test_uninstall_purge_flag_parsing() {
    ...
    #[test]
    fn test_cleanup_base_if_empty_removes_base_when_only_plugins_remain() {
        let tmp = tempfile::tempdir().unwrap();
        let base = tmp.path().join(".cortex");
        fs::create_dir_all(base.join("plugins").join("demo")).unwrap();

        cleanup_base_if_empty(&base);

        assert!(!base.exists());
    }

    #[test]
    fn test_cleanup_base_if_empty_keeps_base_when_instance_remains() {
        let tmp = tempfile::tempdir().unwrap();
        let base = tmp.path().join(".cortex");
        fs::create_dir_all(base.join("plugins").join("demo")).unwrap();
        fs::create_dir_all(base.join("default").join("data")).unwrap();
