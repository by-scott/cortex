...
#
# Commands:
#   install   [--version X.Y.Z]  Download binary + optional systemd service
...
download_binary() {
    ...
    if [ -z "${download_url:-}" ]; then
        ...
        echo "     cargo install --git https://github.com/${REPO}.git --tag v${VERSION}"
        echo ""
...
setup_instance() {
    ...
    # Initialize default instance (generates config.toml with env vars)
    info "Initializing default instance..."
...
print_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  install   [--version X.Y.Z]  Download and install (latest or specific version)"
