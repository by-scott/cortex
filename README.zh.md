...
- 未知插件/MCP 工具默认按保守风险评分处理，并需要确认。生产部署可以补充显式 `[risk.tools.<name>]` 策略，而不是只依赖通用评分。
- Guardrails 会为常见 prompt injection、角色覆盖、泄露和外泄模式返回结构化分类，但仍只是基础防线，不是完整对抗边界。
- 确定性重放依赖记录非确定性副作用。重放投影会使用已记录或 provider 提供的副作用值，但工具调用外部系统时仍需要自己的审计和幂等纪律。

尚未具备：

- 没有原生插件沙箱，也不承诺长期稳定二进制 ABI。
- 没有原生插件热插拔；更新共享库需要重启 daemon。
- 不宣称已经完成多租户加固或对抗输入完备防护。
