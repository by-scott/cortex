...
Cortex 1.5.0 是面向这个方向的 daemon-first 生产 core 重建线。它是对活跃
源码树的主动重写，不是对 1.4 daemon 的渐进整理。旧实现仍然保留在 Git
历史里；当前树只保留显式、可测试、可以直接硬化的机制。

## 当前状态

1.5.0 还不是 1.4 所有用户可见功能的完整替代。它交付的是生产级多用户
runtime 所需的 core contract 和发布纪律；active line 只在 live surface 能落
在这些 contract 之上时恢复它们。
...
- daemon-first Unix socket runtime：bootstrap、status、send、tenant
...
- 通过 SDK contract 执行 runtime tool execution，包含 host-granted
  capability、host-path denial、output-limit enforcement、durable
  side-effect intent/result record；
...
- systemd service setup 和 installer-managed daemon lifecycle；
- HTTP、WebSocket、JSON-RPC、MCP、ACP、Telegram、QQ、browser live client；
- media tool、native plugin loading、process plugin spawning 和旧 skills
  registry；
...
| `cortex-runtime` | 多用户 daemon boundary、tenant/session gate、delivery、ingress、tool execution。 |
...
Cortex 1.5.0 只能从 clean tree 且严格 Docker gate 通过后发布。发布必须包含
SDK crate、tag、GitHub release、Linux binary artifact、checksum 和 gate
evidence。后续 1.5.x 只能在新的 ownership、retrieval、persistence、delivery、
tool-execution 和 gate contract 之上恢复用户可见运行时功能。
